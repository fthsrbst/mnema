...
export interface RetrievalStep {
  name: string;
  execute(candidates: RankedId[], query: string, scope: SearchScope): Promise<RankedId[]> | RankedId[];
}

/** Built-in step: FTS search. */
export const ftsStep: RetrievalStep = {
  name: "fts",
  execute(_candidates, query, scope) {
    const ids = ftsSearch("memories_fts", query, config.searchCandidates, scope);
    return ids.map((id, rank) => ({
      id,
      score: 1 / (RRF_K + rank + 1),
      channels: ["fts" as SearchChannel],
      channel_ranks: { fts: rank + 1 },
    }));
  },
};

/** Built-in step: Vector search. */
export const vectorStep: RetrievalStep = {
  name: "vector",
  async execute(_candidates, query, scope) {
    const ids = await vecSearch("memories_vec", query, config.searchCandidates, scope);
    return ids.map((id, rank) => ({
      id,
      score: 1 / (RRF_K + rank + 1),
      channels: ["vec" as SearchChannel],
      channel_ranks: { vec: rank + 1 },
    }));
  },
};

/** Built-in step: RRF fusion of multiple result lists. */
export const rrfFusionStep: RetrievalStep = {
  name: "rrf_fusion",
  execute(candidates) {
    // Candidates already have scores from previous steps; just sort
    return [...candidates].sort((a, b) => b.score - a.score);
  },
};

/** Built-in step: Diversity boost (penalize same-project clusters). */
export const diversityStep: RetrievalStep = {
  name: "diversity",
  execute(candidates) {
    // Simple diversity: limit same-project results
    const projectCounts = new Map<string | null, number>();
    const result: RankedId[] = [];
    const db = getDb();
    for (const c of candidates) {
      const mem = db.prepare("SELECT project FROM memories WHERE id = ?").get(c.id) as { project: string | null } | undefined;
      const project = mem?.project ?? null;
      const count = projectCounts.get(project) ?? 0;
      if (count >= 5) continue; // Max 5 per project
...
/** Built-in step: Freshness boost (boost recent memories). */
export const freshnessBoostStep: RetrievalStep = {
  name: "freshness_boost",
  execute(candidates) {
    const db = getDb();
    return candidates.map((c) => {
      const mem = db.prepare("SELECT updated_at FROM memories WHERE id = ?").get(c.id) as { updated_at: string } | undefined;
      if (!mem) return c;
      const ageDays = (Date.now() - new Date(mem.updated_at).getTime()) / 86400000;
      const boost = ageDays < 7 ? 1.2 : ageDays < 30 ? 1.1 : 1.0;
...
export const defaultPipeline: RetrievalStep[] = [
  ftsStep,
  vectorStep,
  rrfFusionStep,
];
...
export async function runPipeline(
  query: string,
  scope: SearchScope = {},
  pipeline: RetrievalStep[] = defaultPipeline
): Promise<RankedId[]> {
  let candidates: RankedId[] = [];
  for (const step of pipeline) {
    candidates = await step.execute(candidates, query, scope);
  }
  return candidates;
}
