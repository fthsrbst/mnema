...
| Expensive abuse | Process-local IP/token limits plus shared edge and per-organization usage limits before embeddings and LLM calls | Local header/rate smoke plus deployed load/abuse tests |
...
5. A shared/distributed rate limiter at the public edge and organization-level
   usage metering. The built-in process-local limiter is defense in depth only.
